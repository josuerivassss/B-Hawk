"""Custom /help implementation: cog picker, per-command usage, and paginated listings."""
import discord, pydash as _
from bhawk.ui.paginator import Paginator
from bhawk.kernel import HawkBot, HawkContext, Locale
from discord.ext import commands
from typing import List, Dict

async def get_app_commands_from_cog(cog: str, cache_commands: List[discord.app_commands.AppCommand], bot: HawkBot):
...
class MenuHelpSelect(discord.ui.Select):
    def __init__(self, *, ctx: HawkContext, embed: discord.Embed, commands: List[discord.app_commands.AppCommand], locale: Locale):
        self.ctx = ctx
        self.embed = embed
        self.commands = commands
        self.locale = locale
        self.bot: HawkBot = self.ctx.bot
...
class HelpView(discord.ui.View):
    def __init__(self, *, timeout = 30, embed: discord.Embed, ctx: HawkContext, commands: List[discord.app_commands.AppCommand], locale: Locale):
...
async def send_help(ctx: HawkContext, slash, locale: Locale):
    emb = discord.Embed(description=locale.get("help.intro", botname=ctx.bot.user.name, prefix=ctx.clean_prefix), colour=discord.Color.dark_blue())
...
async def send_help_cog(ctx: HawkContext, cog: str, slash: List[discord.app_commands.AppCommand], locale: Locale):
    cmds = await get_app_commands_from_cog(cog, slash, ctx.bot)
    mapped = map(lambda x: f'**{x["id"]}** : {x["description"]}', cmds)
    _chunk = _.chunk(list(mapped), 10)
    emb = discord.Embed(title=f'{cog} [1/{len(_chunk)}]', colour=discord.Color.dark_blue(), description="\n".join(_chunk[0]))
    emb.set_author(name=f"@{ctx.author.global_name}", icon_url=ctx.author.display_avatar)
    emb.set_footer(text=locale.get("help.footer", prefix=ctx.clean_prefix), icon_url=ctx.bot.user.display_avatar)
    def render(item, page, total):
...
async def send_help_group(ctx: HawkContext, group: commands.HybridGroup, slash: List[discord.app_commands.AppCommand], locale: Locale):
    ...
    emb = discord.Embed(title=group.name, colour=discord.Color.dark_blue(), description=as_slash.description or locale.get("help.noDescription"))
    emb.set_author(name=f"@{ctx.author.global_name}", icon_url=ctx.author.display_avatar)
    emb.set_footer(text=locale.get("help.footer", prefix=ctx.clean_prefix), icon_url=ctx.bot.user.display_avatar)
    if len(mapped) != 0:
...
async def send_help_command(ctx: HawkContext, command: commands.HybridCommand, slash: List[discord.app_commands.AppCommand], locale: Locale):
    as_slash: discord.app_commands.AppCommand = _.find(slash, lambda x: x.name == command.name)
    emb = discord.Embed(title=command.name, colour=discord.Color.dark_blue())
    emb.set_author(name=f"@{ctx.author.global_name}", icon_url=ctx.author.display_avatar)

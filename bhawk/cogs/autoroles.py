"""Autoroles cog: auto-assigns configurable roles to new human/bot members."""
...
class Autoroles(commands.Cog):
    ...
    async def autoroles(self, ctx: HawkContext):
    ...
    async def autoroles_add(self, ctx: HawkContext, role: discord.Role, usertype: Literal["bots", "humans"] = "humans"):
    ...
    @discord.app_commands.describe(usertype="The type of user to assign the role to")
    async def autoroles_remove(self, ctx: HawkContext, role: discord.Role, usertype: Literal["bots", "humans"] = "humans"):
    ...
    async def autoroles_list(self, ctx: HawkContext):
        """Lists the autoroles"""
        await ctx.defer()
        T = await ctx.get_locale()
        ...
        embed = discord.Embed(title="Autoroles", color=discord.Color.dark_blue())
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        for role_id in role_ids:
            ...
            if role:
                try:
                    await member.add_roles(role, reason="Hawk: Autorole assigned on member join")

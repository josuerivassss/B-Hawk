...
(see bhawk/command_registry.py for the ID scheme). Disabling a root
...
from bhawk.command_registry import COG_IDS, COMMAND_IDS, ID_TO_COG, ID_TO_COMMAND, root_id
from discord.ext import commands
from bhawk.kernel import HawkBot, HawkContext, Locale, HawkEmojis, AnswerType
from bhawk.permissions import protected
from bhawk.ui.base import BaseView
import discord
...
def _resolve_target(bot: HawkBot, query: str) -> tuple[str, str, str] | None:
...
def _cascade_ids(bot: HawkBot, kind: str, name: str, target_id: str) -> set[str]:
...
class LanguageMenu(discord.ui.Select):
    def __init__(self, *, ctx: HawkContext, locale: Locale):
        self.ctx = ctx
        self.t = locale
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(T.get("success.languageSet", language=selected_language) + " " + HawkEmojis.Heart, ephemeral=True)
...
class Configuration(commands.Cog):
    ...
    async def _global_disabled_check(self, ctx: HawkContext) -> bool:
    ...
    async def enable(self, ctx: HawkContext, *, target: str):
    ...
    async def disable(self, ctx: HawkContext, *, target: str):
    ...
    async def list_disabled(self, ctx: HawkContext):
    ...
    async def _toggle(self, ctx: HawkContext, target: str, *, enabled: bool) -> None:
        await ctx.defer()
        T = await ctx.get_locale()
    ...
    async def prefix(self, ctx: HawkContext, *, prefix: str):
    ...
    @commands.cooldown(1, 120, commands.BucketType.guild)
    @commands.has_permissions(manage_guild=True)
    async def language(self, ctx: HawkContext):

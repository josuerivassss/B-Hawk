"""Tags cog: per-guild reusable text snippets (create/edit/delete/list/use)."""
...
class TagName(commands.clean_content):
    ...
    async def convert(self, ctx: HawkContext, argument: str) -> str:
...
class Tags(commands.Cog):
    ...
    async def tags(self, ctx: HawkContext, *, tag_name: str):
    ...
    async def tag_view(self, ctx: HawkContext, *, tag_name: str):
        """Views a tag"""
        await ctx.defer()
        T = await ctx.get_locale()
    ...
    async def tag_create(self, ctx: HawkContext, tag_name: Annotated[str, TagName], *, content: str):
        """Creates a new tag"""
        await ctx.defer()
        T = await ctx.get_locale()
    ...
    async def tag_update(self, ctx: HawkContext, tag_name: Annotated[str, TagName], *, new_content: str):
    ...
    async def tag_delete(self, ctx: HawkContext, tag_name: Annotated[str, TagName]):
        """Removes a tag"""
        await ctx.defer()
        T = await ctx.get_locale()
        normalized_name = self.bot.toolkit.normalize(tag_name)
    ...
    async def tag_prune(self, ctx: HawkContext):
    ...
    async def tag_list(self, ctx: HawkContext):

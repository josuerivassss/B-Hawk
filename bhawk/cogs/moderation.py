"""Moderation cog: lockdown, timeout, kick, ban, unban, message purge.

Every action that changes a member's standing is now also written to the
"""
from discord.ext import commands
from bhawk.kernel import HawkBot, HawkContext
from bhawk.timeparse import load
import discord
...
class Moderation(commands.Cog):
    ...
    async def lockdown(self, ctx: HawkContext, channel: discord.TextChannel = None):
    ...
    async def unlockdown(self, ctx: HawkContext, channel: discord.TextChannel = None):
    ...
    async def timeout(self, ctx: HawkContext, member: discord.Member, duration: str, *, reason: str = None):
        ...
        duration = load(duration) # parse duration string into milliseconds
    ...
    async def untimeout(self, ctx: HawkContext, member: discord.Member, *, reason: str = None):
        """Untimeouts a member"""
        await ctx.defer()
        T = await ctx.get_locale()
        if member.id == ctx.author.id:
    ...
    async def clear(self, ctx: HawkContext, amount: int = 10, member: discord.Member = None):
    ...
    async def kick(self, ctx: HawkContext, member: discord.Member, *, reason: str = None):
    ...
    async def ban(self, ctx: HawkContext, member: discord.Member, *, reason: str = None):
    ...
    async def unban(self, ctx: HawkContext, user: discord.User, *, reason: str = None):

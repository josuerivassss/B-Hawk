...
from bhawk.ui.confirmator import Confirmator
...
class Reminders(commands.Cog):
    ...
    async def reminders(self, ctx: HawkContext):
    ...
    def _parse_time(self, time_str: str, user_tz: ZoneInfo | timezone) -> Optional[datetime]:
        ...
        from bhawk.timeparse import load as parse_time
        ...
        try:
            ...
            if hasattr(channel, 'guild') and channel.guild:
                try:
                    ...
                    pass
            embed = discord.Embed(
                title=locale.get("reminders.embed.title") + " " + HawkEmojis.Clock,
    ...
    async def _add_reminder(self, ctx: HawkContext, time: str, message: str, channel: Optional[discord.TextChannel]):
    ...
    async def reminder_add(self, ctx: HawkContext, time: str, *, message: str, channel: Optional[discord.TextChannel] = None):
        """Add a new reminder"""
        await ctx.defer()
    ...
    async def reminder_list(self, ctx: HawkContext, limit: int = 10):
        ...
        embed = discord.Embed(title=T.get("reminders.list.title"), color=discord.Color.dark_blue(), timestamp=datetime.now(self._get_utc()))
        for reminder in reminders:
            remind_at = reminder['remind_at']
            if isinstance(remind_at, str):
                remind_at = datetime.fromisoformat(remind_at.replace('Z', '+00:00'))
    ...
    @commands.cooldown(1, 2, commands.BucketType.member)
    async def reminder_remove(self, ctx: HawkContext, reminder_id: int):
        """Remove a reminder"""
        await ctx.defer()
        T = await ctx.get_locale()
    ...
    @commands.cooldown(1, 2, commands.BucketType.member)
    async def reminder_edit(self, ctx: HawkContext, reminder_id: int, time: Optional[str] = None, message: Optional[str] = None):
        """Edit a reminder"""
        await ctx.defer()
        T = await ctx.get_locale()
        reminder = await self.bot.sql.get(table="reminders", id=reminder_id)
    ...
    async def reminder_prune(self, ctx: HawkContext):
    ...
    @commands.cooldown(1, 2, commands.BucketType.member)
    async def reminder_timezone(self, ctx: HawkContext, *, timezone: str):

...
from bhawk.kernel.context import AnswerType, HawkContext as _EventContext
...
class Tickets(commands.Cog):
    ...
    async def ticket(self, ctx: HawkContext):
    ...
    async def ticket_channel(self, ctx: HawkContext, channel: discord.TextChannel):
        ...
        embed.set_footer(text="Hawk Tickets", icon_url=self.bot.user.display_avatar.url)
        try:
    ...
    async def ticket_role(self, ctx: HawkContext, role: discord.Role):
    ...
    async def ticket_message(self, ctx: HawkContext, *, text: str):
    ...
    async def ticket_enable(self, ctx: HawkContext):
    ...
    async def ticket_disable(self, ctx: HawkContext):
    ...
    async def _build_ticket_embed(self, guild: discord.Guild, user: discord.Member, template: str,
        ...
        embed = discord.Embed(title=locale.get("tickets.embedTitle"), description=body, colour=discord.Color.dark_blue())
        embed.set_image(url=_BANNER_IMAGE_URL)
        embed.set_footer(text="Hawk Tickets", icon_url=self.bot.user.display_avatar.url)
        return embed
    ...
    async def ticket_close(self, ctx: HawkContext):
        ...
        if closed:
            if ctx.interaction:
                await m.edit(content=T.get("tickets.ticketClosed") + HawkEmojis.Heart)
        else:
            await m.edit(content=T.get("tickets.ticketCloseFailed") + HawkEmojis.Crying)
    ...
    async def _close_ticket(self, channel: discord.abc.Messageable, closer: discord.abc.User, interaction: discord.Interaction | None) -> bool:
        ...
        if not isinstance(channel, discord.Thread) or not channel.name.startswith("ticket-"):
            await self._respond(interaction, channel, T.get("errors.notATicket") + " " + HawkEmojis.Crying)
            return False
        ...
        if not (is_opener or has_staff_role or has_manage):
            await self._respond(interaction, channel, T.get("errors.unauthorized") + " " + HawkEmojis.Crying)
            return False

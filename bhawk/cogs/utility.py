...
class Utility(commands.Cog):
    ...
    async def avatar_alias(self, ctx: HawkContext, user: Optional[discord.User]):
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def user(self, ctx: HawkContext):
    ...
    @discord.app_commands.describe(user="The user to show the avatar of")
    async def avatar(self, ctx: HawkContext, user: Optional[discord.User]):
    ...
    async def info(self, ctx: HawkContext, user: Optional[discord.User | discord.Member]):
        ...
        embed = discord.Embed(colour=discord.Color.dark_blue(), title=user.display_name)
    ...
    async def server(self, ctx: HawkContext):
    ...
    async def icon(self, ctx: HawkContext):
        ...
        guild = ctx.guild
        embed = discord.Embed(colour=discord.Color.dark_blue(), title=guild.name)
    ...
    async def server_info(self, ctx: HawkContext):
    ...
    async def banner(self, ctx: HawkContext):
    ...
    async def roles(self, ctx: HawkContext):
    ...
    async def members(self, ctx: HawkContext):
    ...
    async def role(self, ctx: HawkContext, *, role: discord.Role):
    ...
    async def channel(self, ctx: HawkContext, *, channel: Optional[discord.abc.GuildChannel]):
        """Shows information about a channel"""
        await ctx.defer()
        T = await ctx.get_locale()
        if not channel:
            channel = ctx.channel
        embed = discord.Embed(colour=discord.Color.dark_blue(), title=channel.name)
        embed.set_author(name=ctx.guild.name, icon_url=ctx.guild.icon)
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def calendar(self, ctx: HawkContext):
        """Shows a calendar"""
        await ctx.defer()
    ...
    async def rate(self, ctx: HawkContext, source_code: str, target_code: str, amount: float):
    ...
    async def emoji(self, ctx: HawkContext):
    ...
    async def emoji_info(self, ctx: HawkContext, *, emoji: discord.PartialEmoji):
        ...
        embed = discord.Embed(colour=discord.Color.dark_blue(), title=emoji.name)
        embed.set_author(name=ctx.guild.name, icon_url=ctx.guild.icon)
    ...
    @emoji_info.error
    async def emoji_info_error(self, ctx: HawkContext, error):
        T = await ctx.get_locale()
    ...
    async def emoji_image(self, ctx: HawkContext, emoji: str):
    ...
    async def emoji_add(self, ctx: HawkContext, url: Optional[str] = None, name: Optional[str] = "unknown"):
    ...
    async def emoji_remove(self, ctx: HawkContext, emoji: discord.Emoji):
    ...
    async def quote(self, ctx: HawkContext, *, message: discord.Message):
        """Quotes a message"""
        await ctx.defer()
        T = await ctx.get_locale()
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def image(self, ctx: HawkContext, *, query: str):
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def color(self, ctx: HawkContext, hex_code: str):
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def translate(self, ctx: HawkContext, target: str, *, text: str):
        ...
        try:
            embed = discord.Embed(color=discord.Color.dark_blue(), title=f"**__{translator.source.upper()}__ ➜ __{translator.target.upper()}__**", description=translator.translate(text))
            embed.set_author(name=ctx.author.name, icon_url=ctx.author.display_avatar)
            await ctx.send(embed=embed)
        except Exception:
            raise commands.CommandError(T.get("errors.notInfo"), T.get("errors.notInfoHint"))
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def httpstatus(self, ctx: HawkContext, code: int):
        """Shows information about an HTTP status code"""
        await ctx.defer()
        T = await ctx.get_locale()
        try:
            status = HTTPStatus(code)
            await ctx.send(content=f"## **`{status.value}` {status.phrase} {HawkEmojis.Developer}**\n{status.description}\nhttps://http.cat/{status.value}.jpg")
        except ValueError:
    ...
    async def afk(self, ctx: HawkContext, *, reason: str = "AFK"):

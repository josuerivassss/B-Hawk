"""Process entrypoint. `python -m bhawk.main` or the `bhawk` console script.
It spins up a single HawkBot instance, which may be sharded across multiple.
...
e.g.:
    ...
    SHARD_COUNT=8 SHARD_IDS=0,1,2,3 CLUSTER_ID=0 python -m bhawk.main
    # process B (owns shards 4-7)
    SHARD_COUNT=8 SHARD_IDS=4,5,6,7 CLUSTER_ID=1 python -m bhawk.main
...
from bhawk.config import get_settings
from bhawk.kernel.bot import HawkBot
from bhawk.kernel.tree import HawkTreeClass
from bhawk.logging_setup import configure_logging, get_logger
from bhawk import patches
...
BOT_DOMAIN = "hawk.cofue.space"  # used in bot activity and help command footer
...
async def _get_prefix(bot: HawkBot, message: discord.Message) -> list[str] | str:
...
def build_bot() -> HawkBot:
    """Construct a fully configured (but not yet connected) HawkBot."""
...
def run() -> None:
    """Synchronous entrypoint used by the `bhawk` console script."""
    try:
